<script lang="ts">
  ...
  const monthlySubTotal = $derived(
    activeSubs.reduce(
      (sum, s) =>
        sum + toMain(monthlyCost(s.price, s.billingCycle), s.currencyId),
      0
    )
  );
  ...
  const monthExpenseTotal = $derived(
    monthExpenses.reduce((sum, e) => sum + toMain(e.amount, e.currencyId), 0)
  );
  ...
  const budgetUsedPct = $derived(
    data.monthlyBudget
      ? Math.min((monthlySubTotal / data.monthlyBudget) * 100, 100)
      : null
  );
  const budgetRemaining = $derived(
    data.monthlyBudget
      ? Math.max(data.monthlyBudget - monthlySubTotal, 0)
      : null
  );
  ...
  const categoryBreakdown = $derived.by(() => {
    ...
    for (const s of activeSubs) {
      totals.set(
        s.categoryId,
        (totals.get(s.categoryId) ?? 0) +
          toMain(monthlyCost(s.price, s.billingCycle), s.currencyId)
      );
    }
    for (const e of monthExpenses) {
      totals.set(
        e.categoryId,
        (totals.get(e.categoryId) ?? 0) + toMain(e.amount, e.currencyId)
      );
    }
  ...
  const donutTotal = $derived(
    categoryBreakdown.rows.reduce((sum, r) => sum + r.amount, 0)
  );
</script>
...
<div class="space-y-8">
  ...
  <section>
    ...
    <div class="flex gap-3 overflow-x-auto pb-1">
      {#each upcoming as sub (sub.id)}
        ...
        <div
          class="min-w-[140px] rounded-2xl border border-border bg-card p-4 shadow-sm"
        >
          <Logo
            name={sub.name}
            website={sub.website}
            ...
            size={36}
          />
          <p class="mt-3 truncate text-sm font-semibold text-card-foreground">
            {sub.name}
          </p>
          <p class="text-xs text-muted-foreground">{sub.nextRenewal}</p>
          <p class="mt-1 font-bold text-card-foreground">
            {formatMoney(sub.price, subCurrency)}
          </p>
          {#if d <= 7}
            <p
              class="mt-0.5 text-xs {d <= 3
                ? 'text-destructive'
                : 'text-amber-500'}"
            >
  ...
  <section>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
      ...
      {@render statCard(
        "Monthly cost",
        formatMoney(monthlySubTotal, data.mainCurrency)
      )}
      {@render statCard(
        "Yearly cost",
        formatMoney(yearlySubTotal, data.mainCurrency)
      )}
    </div>
  </section>
  ...
  <section>
    <div class="mb-3 flex items-center justify-between">
      ...
      <button
        onclick={() => {
          ...
          if (editingBudget) {
            budgetInput = data.monthlyBudget ? String(data.monthlyBudget) : "";
          }
        }}
    ...
    {#if editingBudget}
      ...
      >
        <input
          ...
          oninvalid={(e) => {
            ...
            input.setCustomValidity(
              data.monthlyBudget
                ? "Monthly budget cannot be empty. Use 0 to remove your budget limit."
                : "Monthly budget cannot be empty."
            );
          }}
          oninput={(e) => {
            const input = e.currentTarget as HTMLInputElement;
        ...
        <button
          type="submit"
          class="flex items-center gap-1 rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground"
        >
    ...
    {#if budgetUsedPct != null}
      <div
        class="grid grid-cols-1 gap-4 rounded-2xl border border-border bg-card p-5 shadow-sm sm:grid-cols-[auto_1fr] sm:items-center sm:gap-8"
      >
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          {@render statCard(
            "Budget",
            formatMoney(data.monthlyBudget ?? 0, data.mainCurrency)
          )}
          {@render statCard(
            "Due this month",
            formatMoney(amountDueThisMonth, data.mainCurrency)
          )}
          {@render statCard(
            overBudget ? "Over budget" : "Remaining",
            overBudget
              ? formatMoney(
                  monthlySubTotal - (data.monthlyBudget ?? 0),
                  data.mainCurrency
                )
    ...
    {:else}
      ...
      <p class="mt-3 text-sm text-muted-foreground">
        Set a monthly budget to track how much headroom you have.
      </p>
    {/if}
  </section>
  ...
  <section>
    <h2
      class="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground"
    >
      Your savings (inactive subscriptions)
    </h2>
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
      {@render statCard("Inactive subscriptions", String(inactiveSubs.length))}
      {@render statCard(
        "Monthly savings",
        formatMoney(monthlySavings, data.mainCurrency),
        "success"
      )}
      {@render statCard(
        "Yearly savings",
        formatMoney(yearlySavings, data.mainCurrency),
        "success"
      )}
    </div>
  </section>
  ...
  <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
    <section class="rounded-2xl border border-border bg-card p-5 shadow-sm">
      ...
      <p class="mb-4 text-xs text-muted-foreground">
        Monthly subscriptions + this month's expenses
      </p>
      {#if donutTotal > 0}
        <Donut
          slices={donutSlices}
    ...
    <section class="rounded-2xl border border-border bg-card p-5 shadow-sm">
      ...
      <p class="mb-4 text-xs text-muted-foreground">
        {monthExpenses.length} entries · {formatMoney(
          monthExpenseTotal,
          data.mainCurrency
        )}
      </p>
      <ul class="space-y-2.5">
        {#each monthExpenses.slice(0, 6) as exp (exp.id)}
          ...
          {@const expCurrency = data.currencies.find(
            (c) => c.id === exp.currencyId
          )}
          <li class="flex items-center gap-3 text-sm">
            ...
            <span class="font-semibold text-card-foreground"
              >{formatMoney(exp.amount, expCurrency)}</span
            >
          </li>
        {:else}
        ...
        {#if monthExpenses.length > 6}
          <li class="pt-1 text-xs text-muted-foreground">
            +{monthExpenses.length - 6} more
          </li>
        {/if}
      </ul>
    </section>
...
{#snippet statCard(
  ...
  <div class="rounded-2xl border border-border bg-card p-4 shadow-sm">
    ...
    <p
      class="mt-1.5 text-xl font-bold {accent === 'destructive'
        ? 'text-destructive'
        : accent === 'success'
