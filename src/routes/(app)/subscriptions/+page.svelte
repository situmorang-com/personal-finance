<script lang="ts">
  ...
  function toggleSort(field: SortField) {
    if (sortField === field) sortDir = sortDir === "desc" ? "asc" : "desc";
      ...
      sortField = field;
      sortDir = field === "renewal" ? "asc" : "desc";
    }
  }
  ...
  const filtered = $derived.by(() => {
    let arr = data.subscriptions.filter((s) => {
      ...
      if (filterCategoryId !== null && s.categoryId !== filterCategoryId)
        return false;
      return true;
    });
    arr = [...arr].sort((a, b) => {
      ...
      if (sortField === "renewal")
        cmp = a.nextRenewal.localeCompare(b.nextRenewal);
      else if (sortField === "price") {
        const aAmt = convertToMain(
          monthlyCost(a.price, a.billingCycle),
          a.currencyId,
          data.currencies,
          data.mainCurrency
        );
        const bAmt = convertToMain(
          monthlyCost(b.price, b.billingCycle),
          b.currencyId,
          data.currencies,
          data.mainCurrency
        );
  ...
  const monthlyTotal = $derived(
    activeSubs.reduce(
      (sum, s) =>
        sum +
        convertToMain(
          monthlyCost(s.price, s.billingCycle),
          s.currencyId,
          data.currencies,
          data.mainCurrency
        ),
...
<div class="space-y-5">
  ...
  {#if form?.error}
    <p
      class="rounded-xl bg-destructive/10 px-4 py-2.5 text-sm text-destructive"
    >
      {form.error}
    </p>
  {/if}
  ...
  <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
    <div class="glass-card glass-card-hover rounded-2xl p-4">
      ...
      <p class="tabular mt-1 text-lg font-bold text-card-foreground">
        {activeSubs.length}
      </p>
    </div>
    <div class="glass-card glass-card-hover rounded-2xl p-4">
      ...
      <p class="tabular mt-1 text-lg font-bold text-card-foreground">
        {formatMoney(monthlyTotal, data.mainCurrency)}
      </p>
    </div>
    <div class="glass-card glass-card-hover rounded-2xl p-4">
      <p class="text-xs text-muted-foreground">Yearly cost</p>
      <p class="tabular mt-1 text-lg font-bold text-card-foreground">
        {formatMoney(yearlyTotal, data.mainCurrency)}
      </p>
    </div>
    <div
      class="glass-card glass-card-hover rounded-2xl p-4 {dueThisWeek.length > 0
        ? 'drop-shadow-[0_0_16px_rgba(245,158,11,0.15)]'
        : ''}"
    >
      <p class="text-xs text-muted-foreground">Due this week</p>
      <p
        class="tabular mt-1 text-lg font-bold {dueThisWeek.length > 0
          ? 'text-amber-600 dark:text-amber-400'
          : 'text-card-foreground'}"
      >
        {formatMoney(dueThisWeekTotal, data.mainCurrency)}
      </p>
      {#if dueThisWeek.length > 0}<p
          class="mt-0.5 text-xs text-muted-foreground"
        >
          {dueThisWeek.length} renewal{dueThisWeek.length !== 1 ? "s" : ""}
        </p>{/if}
    </div>
  </div>
  ...
  <div class="flex flex-wrap items-center gap-2">
    ...
    <div class="relative">
      ...
      >
        ...
        {#if isNonDefault}
          <span
            class="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full bg-amber-400 ring-2 ring-background"
          ></span>
        {/if}
      </button>
      ...
      {#if sortPanelOpen}
        ...
        ></button>
        <div
          class="absolute left-0 top-full z-40 mt-2 w-72 rounded-2xl border border-white/10 bg-popover/80 p-4 shadow-xl backdrop-blur-xl"
        >
              ...
              <div class="space-y-0.5">
                {#each [["renewal", "Renewal"], ["price", "Price"], ["name", "Name"]] as const as [field, label]}
                  ...
                  >
                    <span>{label}</span>
                    {#if sortField === field}
                      {#if sortDir === "desc"}<ArrowDownIcon
                          class="h-3.5 w-3.5"
                        />{:else}<ArrowUpIcon class="h-3.5 w-3.5" />{/if}
                    {/if}
                  </button>
                {/each}
              </div>
                ...
                {#each [["all", "All"], ["active", "Active"], ["paused", "Paused"], ["trial", "Trial"]] as const as [s, label]}
                  <button
                    ...
                    class="flex w-full items-center rounded-lg px-2.5 py-1.5 text-sm transition
											{statusFilter === s
                      ? 'bg-primary/10 font-semibold text-primary'
                      : 'text-card-foreground hover:bg-accent'}">{label}</button
                  >
                {/each}
              </div>
            </div>
  ...
  <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
    {#each filtered as sub, i (sub.id)}
      ...
      {@const subCurrency = data.currencies.find(
        (c) => c.id === sub.currencyId
      )}
      ...
      <div
        ...
        style="animation-delay: {i *
          40}ms; animation-duration: 350ms; animation-fill-mode: backwards;"
      >
        <div class="flex items-center gap-3 p-4 sm:gap-4">
          ...
          <div class="shrink-0">
            <Logo
              name={sub.name}
              website={sub.website}
              color={cat?.color ?? hashColor(sub.name)}
              size={44}
            />
          </div>
          ...
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              ...
              {#if sub.isTrial}
                <span
                  class="flex items-center gap-1 rounded-full bg-violet-500/10 px-2 py-0.5 text-xs font-medium text-violet-600 dark:text-violet-400"
                >
          ...
          <div class="shrink-0 text-right">
            <p
              class="tabular text-lg font-bold tracking-tight text-card-foreground"
            >
              {formatMoney(sub.price, subCurrency)}<span
                class="text-xs font-medium text-muted-foreground"
                >{cycleLabel(sub.billingCycle)}</span
              >
            </p>
            {#if sub.billingCycle !== "monthly" || (sub.currencyId && sub.currencyId !== data.mainCurrency.id)}
              {@const monthlyInMain = convertToMain(
                monthlyEquivalent(sub.price, sub.billingCycle),
                sub.currencyId,
                data.currencies,
                data.mainCurrency
              )}
    ...
    {:else}
      <div
        class="rounded-2xl border border-dashed border-border bg-card py-16 text-center lg:col-span-2"
      >
...
<Sheet.Root bind:open={sheetOpen}>
  <Sheet.Content side="right" class="w-full overflow-y-auto sm:max-w-md">
    <Sheet.Header>
      <Sheet.Title
        >{editing ? "Edit subscription" : "New subscription"}</Sheet.Title
      >
      <Sheet.Description>
        {editing
    ...
    >
      ...
      <label class="flex flex-col gap-1.5 text-sm font-medium">
        Website <span class="text-xs font-normal text-muted-foreground"
          >(used for logo)</span
        >
        <input
          name="website"
      ...
      <label class="flex flex-col gap-1.5 text-sm font-medium">
        Cancel URL <span class="text-xs font-normal text-muted-foreground"
          >(direct link to cancel)</span
        >
        <input
          name="cancelUrl"
          type="url"
          ...
          >
            {#each data.currencies as cur (cur.id)}
              <option
                value={cur.id}
                selected={cur.id ===
                  (editing?.currencyId ?? data.mainCurrency.id)}
              >
                {cur.code} ({cur.symbol})
              </option>
            {/each}
        ...
        <label class="flex flex-col gap-1.5 text-sm font-medium">
          ...
          >
            {#each ["monthly", "yearly", "quarterly", "weekly"] as c (c)}
              <option
                value={c}
                selected={c === (editing?.billingCycle ?? "monthly")}
                class="capitalize">{c}</option
              >
            {/each}
          </select>
        </label>
      ...
      <div class="grid grid-cols-2 gap-3">
        <label class="flex flex-col gap-1.5 text-sm font-medium">
          ...
          >
            ...
            {#each data.categories as cat (cat.id)}
              <option value={cat.id} selected={cat.id === editing?.categoryId}
                >{cat.name}</option
              >
            {/each}
          </select>
        ...
        <label class="flex flex-col gap-1.5 text-sm font-medium">
          ...
          >
            ...
            {#each PAYMENT_METHODS as pm (pm)}
              <option value={pm} selected={pm === editing?.paymentMethod}
                >{paymentLabels[pm]}</option
              >
            {/each}
          </select>
        </label>
